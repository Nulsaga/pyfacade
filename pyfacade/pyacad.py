...
if __name__ == '__main__':
    # ac = Acad("D:\\Coding File\\PyCharm\\pyfacade\\working_file\\JT.dwg")
    # ac = Acad("C:\\Work File\\Python Code\\PycharmProjects\\pyfacade\\working_file\\JT.dwg")
    # ac=Acad()
    # ac.replaceblock('M10_ISO7040_SL-nut_top',
    ...
    # print(res)
    # region ***************************** Tested Sample *************************************

    # pt=ac.get("point", prompt="select first point",bits=1)
    # inp = ac.get("orientation", ref_pnt=pt, prompt="make select or [X-value/Y-value]", bits=0, keyword="X Y")
    # inp=ac.kbinput("real", "input here:", 1, "", space=False)
    # if inp == "":
    #     print("blank input")
    # else:
    #     print(inp)
    # ent = ac.pick(objtype="line", prompt="select one from drawing:", bits=1, keyword="x y")
    # if ent:
    #     if type(ent) != str:
    #         print(ent.ObjectName)
    #     else:
    #         print(ent)
    # else:
    #     print("blank is entered")

    # s = ac.select(objtype=None, prompt="select some regions")
    # if s:
    #     for i in s:
    #         print(i.ObjectName)
    # else:
    ...
    # pt3=[40,-100,0]
    # pt4=[90,0,0]
    # # ls=ac.addcurve(pt1, pt2, pt3, pt4, start_tan=(0.2, 0, 0), end_tan=(-0.2, 0, 0))
    # # r= ac.addrect(pt1, pt2)
    # pt=ac.addnode(pt3, pt4, pt1)
    # rgs = ac.makeregion()
    # print(rgs)
    # for r in rgs:
    #     print(r.ObjectName)
    # pt=[]
    # for i in range(4):
    #     pt.append(ac.get("point"))
    # print(ac.drawing.ActiveLinetype)
    # ac.addcurve(*pt, color="yellow", scale=10)
    # ac.addrect(ac.get("point"), ac.get("point"), color="yellow", scale=10)
    # ac.addcircle(ac.get(), 20, color="yellow", ltype="DASHDOT", scale=0.1)
    # ac.textstyle("test2", "C:\\Windows\\Fonts\\romans.shx", bigfont_file="C:\\Windows\\Fonts\\gbcbig.shx")
    # ac.addmtext("Hello world\nHello you", (5,5,0), height=5, width=50, align_type="MiddleCenter", rotation=1, color="green")

    # Smaple: insert table
    # df = pd.DataFrame({'col_A': [1, 2, None], 'col_B': [None, 'b', 'c'], 'col_D': [None, 20, None]}, index=['r1', 'r2', 'r3'], columns=['col_A', 'col_B', 'col_C', 'col_D'])
    # tb = ac.addtable(df, (0,0,0), 15, 30, title="Table", show_index=True, show_header=True, merge_empty=2, main_align="MiddleCenter",
    #                  cell_color={(2,2):'red'}, layer='test01')
    # dl = [1.1,2,3,None,'A']
    # tb1 = ac.addtable(dl, (200,20,0), [15, 30, 1,1,1,20,1], [20, 40], title="Test Table", show_index=True, show_header=True, acc=4,
    #                  header_textheight=9, main_style='ARIAL', main_align="MiddleCenter", cell_style={(2,1):'ROMANS', (4,0):'ROMANS'},
    #                  cell_color={(2,1):'red', (4,0):(10,250,140)})

    # v = ac.getvector(unit=False)

    # ac.addpolygon((0,0,0), n=8, radius=6, start_angle=0, layer='test01')

    # Sample: get local coordinates of a bolt group
    # bolt = ac.pick(objtype="block", prompt="selection a block at origin point")
    # group_co = ac.locate(block_name=bolt.Name, ucs=ac.UCS(origin=bolt.InsertionPoint))
    # print(np.round(group_co, 3))

    # Sample: read tables of section properties
    # sp = ac.readsecprop(file_name="C:\\Work File\\Python Code\\PycharmProjects\\pyfacade\\working_file\\export.json")
    # print(sp)

    # Sample: get distance from any axis passing centroid to boundary
    # b, ba = ac.getboundary(ac.pick(), spl_sub=500, file_name="secb.json")
    # with open("secb.json") as f:
    #     b_data = json.load(f)
    # u = ac.getvector()  # get measuring direction
    # print(ac.boundalong(b_data['node'], b_data['arc'], u))
    # for reg in ac.select(objtype='region'):
    #     sp = ac.getsecprop(reg, file_name="section_prop.json")
    #     print(sp)

    # Sample: create a section library
    # ac.seclib(file_name="section_lib_transoms.json", sec_name=['Bottom_transom', 'Top_transom'])
    # ac.seclib(file_name="section_lib_mullion.json", sec_name=['RF_C'])
    # back3 = pd.read_json("section_lib.json")

    # Sample: add dimension between 2 points
    # p1 = ac.get()
    # p2 = ac.get(ref_pnt=p1)
    # ac.adddim(p1, p2, offset=-25, dimstyle='DIM')
    # Sample: add overall dimension on a region
    # ac.dimregion(ac.pick(), direction_vector=list(ac.getvector()), dim_offset=-10, outside=True)

    # Sample: Inset a block
    # ac.insertblock((5,4,0), "NUM.dwg", scale=(2, 2, 2), dynamic_prop={7:85.0, 8:pi/4}, attr={0: 15})

    # Sample: mark out element on a section, export to .json file and load
    # ac.lbelem()
    # el = ac.lbelem(dimstyle='DIM', file_name="elems.json", update=False)
    # with open('elems.json') as f:
    #     e_data = json.load(f, parse_int=float)
    # Smaple: set layer while creating object
    # ac.setlayer('test01', hidden=True, freeze=True, activate=True)
    # ac.addcurve(ac.get(), ac.get(), ac.get(), layer='test01')
    # ac.addcircle(ac.get(), radius=20, layer='test02')
    # ac.addnode(ac.get(), layer='test01')
    # ac.fillhatch([[ac.pick()], [ac.pick()]], layer='test01')
    # ac.adddim(ac.get(), ac.get(), offset=15, layer='test01')
    # ac.insertblock((0,0,0), 'NUM.dwg', layer='test01')
    # ac.makeregion(ac.select(), layer='test01')
    # ac.addmtext("AAAA", (0,0,0), height=5, width=25,layer='test01')
    # ac.freedraw(polyline=True,layer='test01')

    # Sample: search by criteria
    # print(*ac.search(lambda x: x.ObjectName == 'AcDbBlockReference' and x.EffectiveName == 'NUM'), sep='\n')

    # Sample: replace blocks
    # ac.replaceblock('222', '222.dwg', offset=(10,-5,0), new_dynamic_prop=None, new_attr=None)
